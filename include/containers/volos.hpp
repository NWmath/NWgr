...
template <int idx, std::unsigned_integral vertex_id_type, typename... Attributes>
class index_adj_list : public unipartite_graph_base, public vector_of_list_of_structs<vertex_id_type, Attributes...> {
public:
  using vertex_id_t    = vertex_id_type;
  using base           = vector_of_list_of_structs<vertex_id_t, Attributes...>;
  using graph_base     = unipartite_graph_base;
  using num_vertices_t = std::array<typename base::size_type, 1>;
  using num_edges_t    = base::size_type;

  index_adj_list(size_t N = 0) : base(N) {}

  index_adj_list(edge_list<directedness::directed, Attributes...>& A) : base(A.num_vertices()) {
    num_edges_ = fill(A, *this);
  }

  index_adj_list(edge_list<directedness::undirected, Attributes...>& A) : base(A.num_vertices()) {
    num_edges_ = fill(A, *this);
  }
  ...
  num_edges_t    num_edges() const { return num_edges_; }

private:
  num_edges_t num_edges_;
};
