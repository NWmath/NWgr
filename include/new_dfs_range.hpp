...
public:
  dfs_range(Graph& graph, vertex_id_t seed = 0) : the_graph_(graph), colors_(graph.end() - graph.begin(), white) {
    Q_.push(seed);
    colors_[seed] = grey;
...
public:
  dfs_edge_range(Graph& graph, vertex_id_t seed = 0) : the_graph_(graph), colors_(graph.end() - graph.begin(), white) {
    Q_.push(seed);
    colors_[seed] = grey;
  }
