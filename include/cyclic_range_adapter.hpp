...
class cyclic_range_adapter
{
 ...
 public:
  ...
  cyclic_range_adapter(Range&& range, Cutoff cutoff)
      : begin_(range.begin()),
        end_(range.end()),
        cutoff_(bgl17::pow2(bgl17::ceil_log2(cutoff)))
  {
  }
  ...
  cyclic_range_adapter(cyclic_range_adapter& rhs, tbb::split)
      : begin_(rhs.begin_),
        end_(rhs.end_),
        cutoff_(rhs.cutoff_),
        cycle_(rhs.cycle_ + rhs.stride_),
        stride_(rhs.stride_ *= 2)
  {
  }
...
template <class Range, class Cutoff>
cyclic_range_adapter(Range range, Cutoff) ->
  cyclic_range_adapter<decltype(range.begin())>;
