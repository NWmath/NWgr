...
#ifndef BOYKOV_KOLMOGOROV2_HPP
#define BOYKOV_KOLMOGOROV2_HPP
  ...
  while (true) {
    ...
    while (!active.empty()) {
      ...
      for (auto it = G[p].begin(); it != G[p].end(); ++it) {
        ...
        if (resid_cap(cf, *ff, *fb) <= tol) continue;
    ...
    for (auto vtx = connect_t; vtx != terminal; vtx = preds[vtx]) {
      path_length++;
      auto edge = trees[vtx];
      min_cap   = std::min(min_cap, resid_cap(std::get<0>(edge), *std::get<1>(edge), *std::get<2>(edge)));
    }
    ...
    while (!orphans.empty()) {
      ...
      if (preds[p] == null_vertex) {
        ...
        for (auto it = G[p].begin(); it != G[p].end(); ++it) {
          ...
          if (!tree_id[q] && resid_cap(std::get<1>(*it), std::get<2>(*it), *std::get<4>(*it)) > tol) {
            active.push(q);
          } else if (tree_id[q] && resid_cap(*std::get<3>(*it), *std::get<4>(*it), std::get<2>(*it)) > tol) {
            active.push(q);
          }
