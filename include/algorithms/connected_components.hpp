...
//      std::cout << "--------vertex--------------" << vtx << " component_ids[vtx]: " << component_ids[vtx] << std::endl;
...
T findDominantComponentID(const std::vector<T>& comp, size_t nsamples = 1024) {
  ...
  auto dominant = std::max_element(sample_counts.begin(),
  sample_counts.end(),
...
std::vector<vertex_id_t> compute_connected_components_v1( Graph& g) {
  size_t      N                        = g.size();
  //std::vector<atomwrapper<vertex_id_t>> comp(N, atomwrapper<vertex_id_t>(-1));
...
#endif
      counting_iterator<vertex_id_t>(0), counting_iterator<vertex_id_t>(g.size()), [&](auto n) { comp[n] = n; });
  //approximate the dominant component by linking certain neighbors of each vertex v (a sparse subgraph)
  std::for_each(
#if defined(EXECUTION_POLICY)
        std::execution::par_unseq,
#endif
        counting_iterator<vertex_id_t>(0), counting_iterator<vertex_id_t>(g.size()), [&](auto u) {
    link(g, u, comp, neighbor_bound);
  });
  compress(comp);
