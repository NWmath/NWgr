...
auto to_two_graph(HyperGraph& H, size_t s = 1) {
  return to_two_graph(std::execution::seq, H, s);
}

template<class ExecutionPolicy, class HyperGraph>
auto to_two_graph(ExecutionPolicy&& exec, HyperGraph& H, size_t s = 1) {
  ...
  for (size_t i = 0; i < n_nbs.size(); ++i) {
    for (size_t j = i + 1; j < n_nbs.size(); ++j) {
      size_t count = nw::graph::intersection_size(n_nbs[i], n_nbs[j]);    //       if intersection_size(n_nbs(i), n_nbs(j)) >= s
      if (count >= s) {
...
template<class ExecutionPolicy, class HyperGraph>
auto base_two(ExecutionPolicy&& exec, HyperGraph& H, size_t s = 1) {

  auto s_over = to_two_graph(exec, H, s);    // 1) find 2-graph corresponding to s-overlapped hyper edges
  ...
  auto s_cc   = ccv1(s_adj);                 // 3) run whatever on new_adjacency
        ...
        for (int j = 0, e = trials; j < e; ++j) {
          switch (id) {
            ...
            case 1:
              ...
              break;
            case 2:
              record([&] { return bfsCC(aos_a, hypernodes, hyperedges); });
              break;
            case 3:
              record([&] { return svCC(std::execution::seq, aos_a, hypernodes, hyperedges); });
              break;
            case 4:
              record([&] { return bfsCC(std::execution::par_unseq, aos_a, hypernodes, hyperedges); });
              break;
            case 5:
              record([&] { return lpaCC(std::execution::seq, aos_a, hypernodes, hyperedges); });
              break;
            case 6:
              record([&] { return base_two(std::execution::seq, aos_a); });
              break;
            default:
